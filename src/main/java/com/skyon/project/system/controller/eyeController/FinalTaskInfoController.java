...
import com.skyon.project.system.domain.eye.QueryLikeParam;
...
@RestController
@RequestMapping("/finalTaskInfo")
public class FinalTaskInfoController extends BaseController {
    ...
    public AjaxResult getSignalManualList(WarningTaskListVo warningTaskListVo) {
        ...
        ||"管理员".equals(roles.get(0).getRoleName())//效果展示，应删除
        ) { // 后续 传页面展示的角色
            // 查询 客户经理 初始时 的 列表。
...
//        }
     ...
     * @RequestParam("devolutionDate") String devolutionDate，
    ...
    public AjaxResult submitTask(@RequestBody TaskInfoSubmitPojo pojo) throws IOException {
        ...
        String devolutionDate = pojo.getDevolutionDate();
        ...
        TaskSubmitService service = (TaskSubmitService) WfDealRoleRegisterFactory.getService(roles.get(0).getRoleName());
        service.taskSubmitMethod(pojo);

        if (RoleName.ACCOUNT_MANAGER.getInfo().equals(roles.get(0).getRoleName())) {

        } else if (RoleName.SUB_BRANCH_SUPERVISOR.getInfo().equals(roles.get(0).getRoleName())) {

        } else if (RoleName.BRANCH_RISK_MONITORING_POST.getInfo().equals(roles.get(0).getRoleName())) {
            // 执行预警认定审批任务
            map.put(WFRole.WFROLE101.getCode(), "9"); // 分行监测审核岗审核 组
        ...
        } else if (RoleName.BRANCH_MONITORING_AUDIT_POST.getInfo().equals(roles.get(0).getRoleName())) {
            map.put(WFRole.WFROLE101.getCode(), "10"); // 分行检测主管审核 组
            // 执行预警认定审批任务
            String taskName = taskWFService.exeTaskByTaskInfoNo(taskInfoNo, String.valueOf(user.getUserId()), map);
        ...
        } else if (RoleName.ACCOUNT_MANAGER.getInfo().equals(roles.get(0).getRoleName())) {
    ...
    public AjaxResult getSignalManualDetail(@PathVariable("taskInfoNo") String taskInfoNo) {
        ...
        DpApTaskInfo dpApTaskInfo =  new DpApTaskInfo();
//                = taskInfoService.selectDpApTaskInfoByTaskInfoNo(taskInfoNo);
    ...
    @GetMapping("/getQueryLike")
    public AjaxResult getSignalManualQueryLike(@RequestBody QueryLikeParam param) {

/*
        List<DpApTaskInfo> list = taskInfoService.getSignalManualQueryLike(param);
*/

        return AjaxResult.success(taskInfoService);
    }
