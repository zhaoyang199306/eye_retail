 ...
 */
@Service
public class WF081SubmitServiceImpl extends TaskCommon implements InitializingBean {

    private static final Logger logger = LoggerFactory.getLogger(WF081SubmitServiceImpl.class);
    ...
    public void afterPropertiesSet() throws Exception {
        WfDealRoleRegisterFactory.register(RoleName.WF_ROLE_081.getCode(), this);
    }

    /**
     * 总行业务部门风险监测岗（金融市场部） 执行任务
     *
     * @param taskNo           任务编号
     * @param code             流程启动编号
     * @param user             用户信息
    ...
    protected Map<String, Object> assembleParam(String taskNo, WfCode code, SysUser user, String processCondition) {
        ...
        map.put(RoleName.WF_ROLE_081.getInfo(), user.getUserId().toString()); //总行业务部门风险监测岗（金融市场部）操作人 id

        if (code == WfCode.WF2002) { // 预警任务审核流程-同业主体预警认定(分行)
            map.put(RoleName.WF_ROLE_091.getInfo(),RoleName.WF_ROLE_091.getCode()); // 总行业务部门监测审核岗（金融市场部） 角色赋值
        }
