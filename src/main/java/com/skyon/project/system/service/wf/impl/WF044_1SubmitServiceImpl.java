 ...
 */
@Service
public class WF044_1SubmitServiceImpl extends TaskCommon implements InitializingBean {

    private static final Logger logger = LoggerFactory.getLogger(WF044_1SubmitServiceImpl.class);
    ...
    public void afterPropertiesSet() throws Exception {
        WfDealRoleRegisterFactory.register(RoleName.WF_ROLE_044.getCode() + "_1", this);
    }
    ...
    protected Map<String, Object> assembleParam(String taskNo, WfCode code, SysUser user, String processCondition) {
        ...
        map.put(RoleName.WF_ROLE_044.getInfo(), user.getUserId()); // 一级分行风险监测审核岗（零售）审核

        if (code == WfCode.WF1101) {

        } else if (code == WfCode.WF2101) { // 预警任务审核流程-自营业务
            map.put("is_054_1", WFTaskCodeFlag.code04(processCondition));
            map.put(RoleName.WF_ROLE_054.getInfo(),RoleName.WF_ROLE_054.getCode()); // 一级分行监测主管(零售)审核
